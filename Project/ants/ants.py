...
class Ant(Insect):
    ...
    def buff(self):
        ...
        if self.is_container() and self.contained_ant:
            if not self.contained_ant.buffed:
                self.contained_ant.damage *= 2
                self.contained_ant.buffed = True
        # END Problem EC
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while cnt <= self.max_range and not now_place is beehive:
            if cnt >= self.min_range and len(now_place.bees) > 0:
                return bee_selector(now_place.bees)  # REPLACE THIS LINE
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"        
        def f(x):
            for target in list_bees:
                target.reduce_health(x)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestage):
        super().action(gamestage)
        beeslist = self.place.bees[:]
        for target in beeslist:
            target.reduce_health(self.damage)
# END Problem 9
...
# END Problem EC
    ...
    def get_behind_ants(self, place, list):
        if place is None:
            return
        if place.ant:
            list.append(place.ant)
        self.get_behind_ants(place.exit, list)
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if not self is QueenAnt.true_queen:
            super().reduce_health(self.health)
        else:
            super().action(gamestate)
            behind_ants = []
            self.get_behind_ants(self.place.exit, behind_ants)
            for i in behind_ants:
                i.buff()
        # END Problem EC
