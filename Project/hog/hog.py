...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    a_digit_one, a_digit_two = get_digit(player_score)
    b_digit_one, b_digit_two = get_digit(opponent_score)
    return a_digit_one < b_digit_one and a_digit_two < b_digit_two
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal: 
        if not who:
            tmp_roll = strategy0(score0, score1)    
            tmp_score = score1
        else:
            tmp_roll = strategy1(score1, score0)
            tmp_score = score0
        tmp_point_getting = take_turn(tmp_roll, tmp_score, dice, goal)
        if not who:
            score0 += tmp_point_getting
            tmp_player = score0
            tmp_opponent = score1
        else:
            score1 += tmp_point_getting
            tmp_player = score1
            tmp_opponent = score0
        if not more_boar(tmp_player,tmp_opponent):
            who = next_player(who)
        # BEGIN PROBLEM 6
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if not who:
            ...
            now_score = score1
        tmp = now_score - last_score
        if tmp > running_high:
            print('Player', who, 'has reached a new maximum point gain.', tmp, 'point(s)!')
        high_score = max(tmp,running_high)
        return announce_highest(who, now_score, high_score)
    return say
    # END PROBLEM 7
...
def piggypoints_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if x >= cutoff:
        res = 0
    return res # Replace this statement
    # END PROBLEM 10
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    res = 0
    x = piggy_points(opponent_score)
    judge = more_boar(score+x, opponent_score)
    if not judge:
        res = piggypoints_strategy(score, opponent_score, cutoff, num_rolls)
    return res  # Replace this statement
    # END PROBLEM 11
