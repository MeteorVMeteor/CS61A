...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    tmp_city = make_city('DesCity', lat, lon)
...
def sprout_leaves(t, leaves):
    ...
    def f(t):
        if is_leaf(t):
            ...
            return tree(label(t), [f(b) for b in branches(t)])
    new_tree = f(t)
    print("DEBUG:"+ str(new_tree))
    return new_tree
...
def add_trees(t1, t2):
    ...
    def f(t1 = [0], t2 = [0]):
        if is_leaf(t1) or is_leaf(t2):
            ...
            tmptree = []
            tmp = zip(branches(t1), branches(t2))
            for i in tmp:
                if len(i) < 2:
                    tmptree += f(i, [0])
                else:
                    tmptree += f(i[0], i[1])
            return tree(label(t1)+label(t2),tmptree)
    new_tree = f(t1, t2)
    return new_tree
